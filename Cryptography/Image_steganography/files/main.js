...
var importImage = function(e) {
	...
	console.log("e"+e);// object event
	
	//FileReader object we can now load the image into the DOM
	...
	reader.onload = (event)=>{
		...
		//With our nifty data URL we can now load it into an Image object, which we'll then feed into our canvas element to do the pixel manipulation.
...
var encode = function() {
			...
			if (password.length > 0) {
				//encrypting the message before hiding it in the image. To do this, we'll use the SJCL library. Its encrypt function uses reasonable defaults (AES-128 in CCM mode and PBKDF2 with 1000 iterations).
				
				message = sjcl.encrypt(password, message);//https://crypto.stanford.edu/sjcl/
				
				console.log("encyption password"+message);
			...
			//imgData 0-255 gives color values ....
...
var getMessageBits = function(message) {
    ...
    console.log("messagetextBits="+messageBits);
    return messageBits;
};
